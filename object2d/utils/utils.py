...
def ap_per_class1(tp, conf, pred_cls, target_cls):
    """ Compute the average precision, given the recall and precision curves.
    Source: https://github.com/rafaelpadilla/Object-Detection-Metrics.
    # Arguments
        tp:    True positives (list).
    ...
    for c in unique_classes:
        ...
        if n_p == 0 and n_gt == 0:
            ...
            # AP from recall-precision curve
            ap.append(compute_ap2(recall, precision,ap2,conf))
...
def compute_ap(recall, precision):
    ...
    return ap
def compute_ap2(recall, precision,ap2,conf):
    """ Compute the average precision, given the recall and precision curves.
    Source: https://github.com/rbgirshick/py-faster-rcnn.
    # Arguments
        recall:    The recall curve (list).
    ...
    # Append sentinel values to beginning and end
    conf = np.concatenate(([0.], conf, [1.]))
    ap2 = np.concatenate(([0.], ap2, [0.]))
    # 对置信度设置不同的阈值，然后计算该阈值之下的ap2的
    # Compute the precision envelope
    for i in range(ap2.size - 1, 0, -1):
        ap2[i - 1] = np.maximum(ap2[i - 1], ap2[i])

    # Integrate area under curve
    method = 'interp'  # methods: 'continuous', 'interp'
    if method == 'interp':
        x = np.linspace(0, 1, 101)  # 101-point interp (COCO)
        ap = np.trapz(np.interp(x, conf, ap2), x)  # integrate
    else:  # 'continuous'
        i = np.where(conf[1:] != conf[:-1])[0]  # points where x axis (recall) changes
        ap = np.sum((conf[i + 1] - conf[i]) * ap2[i + 1])  # area under curve
...
def realTimePlotResults(results,tv_img_path,dpi=200):
    from IPython import display
    s = ['GIoU', 'Objectness', 'Classification', 'Train loss',
    ...
    if len(results)==4:
        ...
        for i in range(len(results)):
            x = range(len(results[i]))
            y = results[i]
            ax[i].plot(x, y)
            ax[i].set_title(s[i])
        fname = os.path.join(tv_img_path , "trainLoss{}.jpg".format(curtime))
    else :
        ...
        for i in range(7):
            ...
            y = results[i]
            ax[i].plot(x, y)
            ax[i].set_title(s[i+4])
        fname = os.path.join(tv_img_path, "testLoss{}.jpg".format(curtime))
    fig.tight_layout()
    # ax[1].legend()
    # plt.show()
    # display.clear_output(wait=True)
    if not os.path.exists(tv_img_path):
        os.makedirs(tv_img_path)
